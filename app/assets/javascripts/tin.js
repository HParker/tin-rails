...
Elm.AutoComplete.make = function (_elm) {
   ...
   var fetch = function ($package) {
      return $Effects.task(A2($Task.map,StoreCompletions,$Task.toMaybe(A2($Http.get,decodeCompletion,A2($Basics._op["++"],"/completion/?pack=",$package)))));
   };
...
Elm.Cards.make = function (_elm) {
   ...
   var getCard = function (command) {
      return $Effects.task(A2($Task.map,Add,$Task.toMaybe(A2($Http.get,$Card.decode,A2($Basics._op["++"],"api/?q=",command)))));
   };
   var update = F2(function (action,model) {
      var _p0 = action;
      switch (_p0.ctor)
